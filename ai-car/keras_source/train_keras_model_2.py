...
def main():
    ...
    #新增left_L路牌
    match_left_L = os.path.join(args.data_dir, 'left_L', '*.jpg') 
    paths_left_L = glob.glob(match_left_L)

    match_test = os.path.join(args.data_dir, 'test', '*.jpg')
    paths_test = glob.glob(match_test)

    n_train = len(paths_left) + len(paths_right) + len(paths_stop) + len(paths_other) + len(paths_left_L)
    ...
    # 讀取圖片到資料集
    paths_train = paths_left + paths_right + paths_stop + paths_other + paths_left_L
    ...
    end_ind   = n_left + n_right + n_stop + n_left_L 
    label[begin_ind:end_ind, 3] = 1.0
    
    begin_ind = n_left + n_right + n_stop + n_left_L 
    end_ind   = n_left + n_right + n_stop + n_left_L + n_other
